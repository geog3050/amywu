...
vals = input('Enter values, separated by comma and a space: ')

as_list = [float(i) for i in vals.split(', ')]
...
#       9951.5
######################################################################
as_list.sort() # sort list

def find_avg(list_input):
    mysum=0
    for i in range(len(list_input)):
        mysum = list_input[i] + mysum
    return mysum/len(list_input)

if mode=="min":
    print(as_list[0]) # return first elem in sorted list
elif mode=="max":
    print(as_list[-1]) # return last elem in sorted list
elif mode=="avg":
    print(find_avg(as_list))
